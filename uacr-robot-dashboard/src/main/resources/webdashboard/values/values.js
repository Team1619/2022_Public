...
//A boolean tracking whether the main menu is open
...
//Tracks the current driver and operator gamepads
let driverGamepadIndex = -1;
let operatorGamepadIndex = -1;

//If a gamepad connects update the list of available gamepads
window.addEventListener("gamepadconnected", updateGamepadList);

//If a gamepad disconnects update the list of available gamepads
    ...
    for (let g = 0; g < gamepads.length; g++) {
        try {
            ...
            if (gamepad.id.toLowerCase().includes('x')) {
                ...
                //Create a name for the gamepad
                let name = "Xbox One Controller";

                if(gamepad.id.includes("360")) {
                    name = "Xbox 360 Controller";
                }

                //Add the gamepad number and name to the driverSelector
                driverSelector.innerHTML += "<option value=\"" + g + "\">" + gamepadNumber + " - " + name + "</option>";

                //Add the gamepad number and name to the driverSelector
                operatorSelector.innerHTML += "<option value=\"" + g + "\">" + gamepadNumber + " - " + name + "</option>";
...
function closeMenu() {
    ...
    driverGamepadIndex = parseInt(document.getElementById('driverSelector').value);
    operatorGamepadIndex = parseInt(document.getElementById('operatorSelector').value);
...
function openUpdateValue(type, name, value) {
    ...
    if (type === "output") {
        ...
        document.getElementById('inputVectorSelector').style.display = "none";
    } else if (type === "vector") {
        ...
        document.getElementById("inputVectorSelector").innerHTML = "";
        ...
        for (let v of values) {
            ...
            document.getElementById("inputVectorSelector").innerHTML += "<option value=\"" + val + "\">" + val + "</option>";
        }
        ...
        document.getElementById('inputVectorSelector').style.display = "block";
    } else {
...
function openVectorGraphingMenu() {
    ...
    document.getElementById("vectorGraphingX").innerHTML = "<option value=\"time\">time</option>";
...
function createDraggableViewer(name, left, top) {
    ...
    div.ondblclick = function (e) {
        ...
        //Prevent the default menu open operation
        e.stopPropagation();
...
function sendUpdateValue(type, name, value) {
    if (type === "vector") {
        ...
        //Get the selected part of the vector from the inputVectorSelector on the value menu
        let selected = document.getElementById("inputVectorSelector").value;
    ...
    socket.onmessage = function (message) {
        ...
        switch (messageData.get("response")) {
            case "values":
                ...
                //Get the graph values from session storage
                let graphValues = JSON.parse(sessionStorage['graph_values']);
                ...
                loop: for (let v of values) {
                    ...
                    if (parts[1] === "ips_selected_auto") {
                        //If the name of the value is ips_selected_auto then add it to sessionStorage,
                    ...
                    if (parts[0] === "vector" && parts[1].startsWith("gr_")) {
                        ...
                        for (let v = 0; v < valueParts.length; v += 3) {

                            //Add each of the x,y points to the graph dataset
                            vectorGraphValues.push({
                                h: valueParts[v].split(": ")[1],
                                x: valueParts[v + 1].split(": ")[1],
                                y: valueParts[v + 2].split(": ")[1]
                            });
                        }
                    ...
                    if (graphValues[parts[1]] !== undefined) {
                        ...
                        if (parts[0] === "vector") {
                            ...
                            if (xValue === undefined) {
                                graphValues[parts[1]].value = yValue;
                            } else {
...
function updateGamepads() {
    ...
    if(mainMenuOpen) {
        for (let option of document.getElementById('driverSelector').childNodes) {
            let value = parseInt(option.value);
            if (value >= 0) {
                let pressed = false;
                for (let button of gamepads[value].buttons) {
                    pressed = pressed || button.pressed;
                }
                if (pressed && !option.innerText.includes("* ")) {
                    option.innerText = "* " + option.innerText;
                }
                if (!pressed && option.innerText.includes("* ")) {
                    option.innerText = option.innerText.substring(2, option.innerText.length);
                }
            }
        }

        for (let option of document.getElementById('operatorSelector').childNodes) {
            let value = parseInt(option.value);
            if (value >= 0) {
                let pressed = false;
                for (let button of gamepads[value].buttons) {
    ...
    let gamepad = gamepads[driverGamepadIndex];
...
function keyEvent(key, value) {
    ...
    if (driverGamepadIndex === -2) {
        ...
        sendUpdateValue(type, (type === "boolean" ? "ipb_driver_" : "ipn_driver_") + name, value);
    }

    if (operatorGamepadIndex === -2) {
        //If the keyboard gamepad is selected for the operator,
        //use the value updating code to send the value to the robot code,
        //with the type, name, and value of the gamepad update event
        sendUpdateValue(type, (type === "boolean" ? "ipb_operator_" : "ipn_operator_") + name, value);
    }
}
...
function updateGamepadNumericEvent(gamepad, user, id, value) {
    ...
    if (name.includes('trigger')) {
        ...
        //with the type, name, and value of the gamepad update event
        sendUpdateValue('boolean', name.replace('ipn_', 'ipb_'), value > 0.5);
    }
...
//Initially populate the gamepad list
updateGamepadList();
