...
function setBooleanBox(name, value, text) {
...
function addLog(name, type, message) {
    ...
    if ($("#" + id)[0] === undefined) {
        ...
        //Add the value display to the valuesContainer
        document.getElementById("valuesContainer").appendChild(logArea);
    }
    ...
    if (type === "INFO") {
        //If the message is an info message,
        //add the infoLine css class to the div so it is styled correctly
        div.classList.add("infoLine");
    } else if (type === "ERROR") {
        ...
        //If there is an error line set the background of the log to red to alert the drivers
        $("#" + id).css("background-color", "var(--yellow)");
    }
            ...
            case "match_values":
                ...
                for (let v of values) {
                    ...
                    if (parts[0] === "value") {
                        ...
                        //Get log messages from the UrlFormData
                        let rawMessages = parts[2];

                        //Split each message
                        let messages = rawMessages.split("TYPE:");

                        //Loop through all of the messages
                        for (let rawMessage of messages) {

                            //Split the raw message data into the type and message
                            let messageParts = rawMessage.split("MESSAGE:");

                            //Get the type and message from the message parts
                            let type = messageParts[0];
                            let message = messageParts[1];

                            //If this message isn't correctly formatted skip it
                            if (type === undefined || message === undefined) {
                                continue;
                            }

                            addLog(parts[1], type, message);
                        }
                    } else if (parts[0] === "auto") {
                        setBooleanBox(parts[1], parts[2] !== "No Auto", parts[2]);
                    } else if (parts[0] === "other") {
                        switch (parts[1]) {
                            case "Limelight":
                                if(parts[2] === "true") {
                                    $("#limelightLocked").css("color", "var(--green)");
                                } else {
                                    $("#limelightLocked").css("color", "var(--red)");
                                }
                                break;
                            case "Flywheel":
                                if(parts[2] === "true") {
                                    $("#flywheelPrimed").css("color", "var(--green)");
                                } else {
                                    $("#flywheelPrimed").css("color", "var(--red)");
                                }
                                break;
                            case "Hood":
                                if(parts[2] === "true") {
                                    $("#hoodAligned").css("color", "var(--green)");
                                } else {
                                    $("#hoodAligned").css("color", "var(--red)");
                                }
                                break;
                            case "Primed":
                                if(parts[2] === "true") {
                                    $("#mainCamera").css("border-color", "var(--green)");
                                } else {
                                    $("#mainCamera").css("border-color", "var(--red)");
                                }
                                break;
                        }
                    }
