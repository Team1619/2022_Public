...
public abstract class BaseOdometry extends InputVector {
    ...
    private Pose2d currentPositionAcc;
    ...
    public void update() {
        ...
        Vector currentPoint = new Vector(new Vector(rawPosition).rotate(HeadingMode.ROBOT_CENTRIC == headingMode ? positionOffset.getHeading() : 0).add(positionOffset));
        double currentHeading = rawPosition.getHeading() + positionOffset.getHeading();
        currentPosition = new Pose2d(currentPoint, rangeAngle(currentHeading));
        currentPositionAcc = new Pose2d(currentPoint, currentHeading);
        deltaPosition = new Vector(currentPosition.subtract(lastPosition));
    }

    @Override
    public Map<String, Double> get() {
        return Map.of("x", currentPosition.getX(), "y", currentPosition.getY(), "dx", deltaPosition.getX(), "dy", deltaPosition.getY(), "heading", currentPosition.getHeading(), "heading_acc", currentPositionAcc.getHeading());
    }
