...
import org.team1619.models.inputs.vector.*;
...
import org.team1619.models.outputs.numeric.robot.RobotRoboRioRelay;
...
public class AbstractRobotModelFactory extends AbstractModelFactory {
    ...
    public OutputNumeric createOutputNumeric(Object name, Config config, YamlConfigParser parser) {
        ...
        switch (config.getType()) {
            ...
            case "roborio_relay":
                return new RobotRoboRioRelay(name, config, sharedHardwareFactory);
            default:
    ...
    public OutputBoolean createOutputBoolean(Object name, Config config, YamlConfigParser parser) {
        ...
        switch (config.getType()) {
            case "solenoid_single":
                return new RobotSolenoidSingle(name, config, sharedHardwareFactory, sharedRobotConfiguration);
            case "solenoid_double":
                return new RobotSolenoidDouble(name, config, sharedHardwareFactory, sharedRobotConfiguration);
            default:
    ...
    public InputVector createInputVector(Object name, Config config) {
        ...
        switch (config.getType()) {
            ...
            case "limelight_odometry":
                return new LimelightOdometry(name, config, sharedInputValues);
            ...
            case "rev_color_sensor_v3":
                return new RobotRevColorSensorV3(name, config, sharedHardwareFactory);
            case "power_distribution_panel":
                return new RobotPowerDistributionPanel(name, config, sharedHardwareFactory);
            default:
