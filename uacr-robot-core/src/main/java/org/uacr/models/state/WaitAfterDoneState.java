...
public class WaitAfterDoneState implements State {
    ...
    private static final Logger logger = LogManager.getLogger(WaitAfterDoneState.class);

    private final Timer stateTimer;
    private final Timer maxTimer;
    private final State subState;
    private final int waitAfterDoneTime;
    ...
    public WaitAfterDoneState(AbstractModelFactory modelFactory, String name, YamlConfigParser parser, Config config) {
        stateTimer = new Timer();
        maxTimer = new Timer();
        waitAfterDoneTime = config.getInt("wait_after_done_time");
    ...
    public boolean isDone() {
        ...
        if (subState.isDone()) {
            if (!stateTimer.isStarted()) {
                stateTimer.start(waitAfterDoneTime);
            }
