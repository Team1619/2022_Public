...
public abstract class Path {
     ...
     */
    protected final ArrayList<Pose2d> waypoints;

    protected final ArrayList<PathDeviationConfig> deviations;

    @Nullable
    protected ArrayList<Pose2d> generatedPoints;

    /**
     * All the points along the path, interpolated from the waypoints
     */
    @Nullable
     ...
     */
    public Path(ArrayList<Pose2d> waypoints) {
    ...
    /**
     * Returns all pathpoints in path (path).
     *
     * @return a PathPoint ArrayList
     */
    public ArrayList<PathPoint> getPathPoints() {
        if (pathPoints != null) {
            return (ArrayList<PathPoint>) pathPoints.clone();
        }
        build();
    ...
    /**
     * Returns all waypoints in path (path).
     *
     * @return a PathPoint ArrayList
     */
    public ArrayList<Pose2d> getWayPoints() {
        if (waypoints != null) {
            return (ArrayList<Pose2d>) waypoints.clone();
        }
        build();
        return getWayPoints();
    }
    ...
    public double getAngleFromPathPoint(int index, Pose2d currentLocation) {
        if (pathPoints == null || pathPoints.size() == 0) return 0.0;
    ...
    public double getCurvatureFromPathPoint(int index, Pose2d currentLocation) {
        ...
        Vector delta = new Vector(index < getPathPoints().size() - 1 ? getPathPoint(index).subtract(currentLocation) : getPathPoint(getPathPoints().size() - 1).add(new Vector(getPathPoint(getPathPoints().size() - 1).subtract(getPathPoint(getPathPoints().size() - 2))).normalize().scale(lookAheadDistance - currentLocation.distance(getPathPoint(getPathPoints().size() - 1)))).subtract(currentLocation));
    ...
    public double getPathPointVelocity(int index, Pose2d currentLocation) {
        ...
        for (int i = index; i < index + velocityLookAheadPoints && i < getPathPoints().size(); i++) {
    ...
    public int getClosestPointIndex(Point currentPosition) {
        if (pathPoints == null || pathPoints.size() == 0) return -1;

        double distance = Double.POSITIVE_INFINITY;
        int index = -1;

        for (int i = lastCurrentPointIndex; i < getPathPoints().size(); i++) {
    ...
    public int getLookAheadPointIndex(Pose2d currentPosition, int closest) {
        ...
        if(0 <= closest && closest < pathPoints.size()) {
            ...
            if (closest != getPathPoints().size() - 1) return getPathPoints().size() - 1;
        }
    ...
    protected void fill() {
        ...
        generatedPoints = (ArrayList<Pose2d>) waypoints.clone();
    ...
    protected void smooth() {
        ...
        while (change / changedPoints >= 0.01) {
            ...
            ArrayList<Pose2d> newPoints = (ArrayList<Pose2d>) generatedPoints.clone();

            for (int i = 1; i < generatedPoints.size() - 1; i++) {
                Pose2d point = generatedPoints.get(i);
                ...
                Pose2d newPoint = point.add(delta.normalize().scale(delta.magnitude() * pathSmoothing));
    ...
    private void createPath() {
        ...
        for (int p = 0; p < generatedPoints.size(); p++) {
            pathPoints.add(new PathPoint(generatedPoints.get(p), getPointDistance(p), getPointCurvature(p)));
        }
