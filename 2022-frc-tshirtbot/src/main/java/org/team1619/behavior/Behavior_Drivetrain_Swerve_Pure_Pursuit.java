...
//            graphVelocityProfile(stateName, currentPath);
    ...
    public void update() {
        ...
        setModulePowers(new Vector(currentPath.getPoint(lookahead).subtract(currentPosition)).normalize().scale(velocity).rotate(-currentPosition.getHeading()), headingMode, currentPath.getPoint(closest).getHeading());
    }
