    ...
    protected void setModulePowers(Vector translation, String headingMode, double headingOutput, String headingProfile) {
        ...
        switch (headingMode) {
            case "navx":
                ...
                // from the IMU when an auto path defines a non-zero start heading. Use the accumulating heading value
                // to avoid discontinuity of the heading in the PID controller.
                setModulePowers(translation, headingController.getWithPID(sharedInputValues.getVector(odometry).get("heading_acc")));
                break;
            case "limelight":
